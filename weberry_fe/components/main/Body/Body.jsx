...
const Body = (props) => {
  ...
  if (reports) {
    ...
    for (let i = 0; i <= reports.length; i++) toggleConfig['top'][i] = `top-${24 + 4 * i}`
  }
      ...
      <div className='m-0 p-7 pt-0 text-sm'>
            ...
            <div className='flex justify-center'>
              <div className='m-2 bg-yello_100  max-w-7xl ml-7 mr-7 rounded-lg'>
                <div className='p-6 box-content relative'>
                  <div className="absolute top-1/2 left-0 border-2" onClick={prevImageno}>&#60;</div>
                  <div className="absolute top-1/2 right-0 border-2" onClick={nextImageno}>&#62;</div>
                  {reports && reports.length > 0 && <>
                              <details id='toggle' className='absolute flex flex-col items-start left-20 open:bg-white'>
                                <summary >
                                  <span>
                                    {point === reports.length ? <strong className='text-base mb-2'> {reports[0].data.farm.farmId}: 전체</strong>
                                                              : <strong className='text-base mb-2'> {reports[point].data.farm.farmId}: {reports[point].data.point}</strong>}
                                  </span>
                                </summary>
                                    {reports.map((report, idx) => <strong className={`text-base ml-4 mt-1`} id={idx} onClick={chooseReport}> {report.data.farm.farmId}: {report.data.point}</strong>)}
                                    <strong className={`text-base ml-10 mt-1`} id={reports.length} onClick={chooseReport}> {reports[0].data.farm.farmId}: 전체</strong>
                              </details>
                              <img className='mt-12' src={imageList[imageno]}/>
                              <a className='indent-8'>{reports[parseInt(imageno / 2)].status}</a>
                              </>}
                </div>
              </div>
            </div>
        ...
        <div className='text-lg font-black '>
          <div className='m-8 mb-16  mt-8'>
            <svg xmlns="http://www.w3.org/2000/svg" width="26" height="26" fill="#FFA778" class="bi bi-brightness-high-fill" viewBox="0 0 16 16">
            ...
            온도
            <span className='ml-5 border-2  border-grey rounded-md p-4 text-berry '>{reports && reports.length > 0 && point === reports.length? reports[parseInt(imageno / 2)].data.temperature
                                                                                         : reports && reports.length > 0 ? reports[point].data.temperature : '??'}</span>
          </div>
          ...
          <div className='m-8 -mt-2'>
            ...
            습도
            <span className='ml-5 border-2  border-grey rounded-md p-4 text-water'>{reports && reports.length > 0 && point === reports.length? reports[parseInt(imageno / 2)].data.humidity
                                                                                        : reports && reports.length > 0 ? reports[point].data.humidity :'??'}</span></div>
        </div>
      </div>
    </div>
  )
